...
! ******************************************************************************
!  
! $Id: RFLU_ModStanley.F90,v 1.1.1.1 2015/01/23 22:57:50 tbanerjee Exp $
!
! Purpose: Compute internal energy for mixture of detonation PRODUCTS AND AIR 
!
! Description: None.
!
! Input: None.
...
! ******************************************************************************
  ...
  SUBROUTINE ComputeEngGasMix(r,p,Y,e,a,T)
...
! ******************************************************************************
  ...
  DOUBLE PRECISION, INTENT(INOUT) :: r, p, Y
  DOUBLE PRECISION, INTENT(OUT):: e, a, T

! ******************************************************************************
! Locals
! ******************************************************************************
  INTEGER :: iter, iterMax, i
  DOUBLE PRECISION :: tol,tolok,smallval, smallscale,vap,vbp,rap,rbp,Tap,Tbp, ebp,fp
  DOUBLE PRECISION :: s, f, df, v, va, vb, ea, eb, alpha
  DOUBLE PRECISION :: h, dummyReal,ra, rb, Ta, Tb, aa, ab, hb, pa,pb
...
! ******************************************************************************
! Both species ideal gas 
! ******************************************************************************
!  IF ( EOSFlag == 1 ) THEN
!    gammaMix = (shcpLeft*Y + shcpRight*(1.0D0-Y)) &
!              /(shcvLeft*Y + shcvRight*(1.0D0-Y))
!    e = p/r/(gammaMix-1.0D0)
!    a = SQRT(gammaMix*p/r)
!    RETURN
!  END IF !EOSFlag 
...
! explosive gas 
! ******************************************************************************
  IF ( ABS(1.0D0 - Y) <= 1.0D-8 ) THEN 
    CALL JWL2(p,r,e,a,T,3)
 if(e.le.0.0) write(*,*) 'in JWL2:Y,p,r,e,T,a:',Y,p,r,e,T,a
   RETURN
  END IF ! Y
...
! ******************************************************************************
! explosive gas and air mixture
! ******************************************************************************
! ==============================================================================
! Initial Guess
! ==============================================================================
  va = v !6.213065525944548D0 !v*8.0D0
...
! ==============================================================================
  ...
  DO iter = 1,iterMax
    ...
    ra = 1.0D0/va
    rb = 1.0D0/vb
    CALL JWL2 (p,ra,dummyReal,dummyReal,Ta,2)
    CALL tgas5(p,rb,eb)
    CALL tgas1(eb,rb,pb,dummyReal,Tb,3) 
    f  = Ta - Tb

    smallscale = smallval*va
!   NOTE: the small number added on va must gurantee reasonable vbp (postive, not too big)
    IF ( (va+smallscale)*Y > v ) smallscale = -smallscale 
    vap = va + smallscale	
    vbp = (v-vap*Y)/(1.0D0-Y)	
    rap = 1.0D0/vap
    rbp = 1.0D0/vbp
    CALL JWL2 (p,rap,dummyReal,dummyReal,Tap,2)
    CALL tgas5(p,rbp,ebp)
    CALL tgas1(ebp,rbp,pb,dummyReal,Tbp,3) 
    fp = Tap - Tbp
    df = (fp-f)/smallscale
    s = -f/df

! TEMPORARY
    IF ( Tap < 0.0D0 .OR. Tbp < 0.0D0 ) THEN 
      WRITE(*,*) 'Error: negative temperature', Y, smallscale, vap, vbp, va, vb, v
      STOP
    END IF !T
! END TEMPORARY
 
    tol = tol/MIN(Y,(1.0D0-Y))
    IF ( ABS(s) < tol ) THEN
      EXIT 
    ELSE IF ( ABS(s) < 1.0D-3) THEN  
      alpha = 1.0D0
    ELSE IF ( ABS(s) < 1.0D-2 ) THEN 
      alpha = 0.8D0
    ELSE 
      alpha = 0.4D0
    END IF !s
...
! Note: gurantee vb >0
    term = va*Y - v 
    IF ( term > 0.0D0 ) va = va - s*alpha/2.0D0 

  END DO ! iter
!  IF ( ABS(s) > tolok ) THEN 
!    WRITE(*,*) 'Secant Method Iteration for energy calculations is not converged!',s, Y
!    STOP
!  END IF ! iter
...
! ==============================================================================
  ...
  ra = 1.0D0/va
  rb = 1.0D0/vb
  CALL JWL2 (p,ra,ea,aa,T,3)
  CALL tgas5(p,rb,eb)
  CALL tgas1(eb,rb,pb,ab,Tb,3) 
  IF ( ABS(T-Tb) > tolok*T/MIN(Y,(1.0D0-Y)) )  THEN
    WRITE(*,*) 'Secant Method Iteration may not be converged!'
    WRITE(*,'(4(E15.8,1X))') T,Tb,s,Y
!    STOP
  END IF ! T 
  e  = ea*Y + eb*(1.0D0-Y)
  a  = aa*Y + ab*(1.0D0-Y)
...
! End
! ******************************************************************************
  END SUBROUTINE ComputeEngGasMix
...
! $Id: RFLU_ModStanley.F90,v 1.1.1.1 2015/01/23 22:57:50 tbanerjee Exp $
!
! Purpose: Compute pressure and other thermal properties for detonation products 
!          and air, and their mixture 
!
! Description: JWL eos is used for detonation products, and curve-fit by 
!              Prof. Tannehill is used for air (equilibrium). 
!
! Input: None.
!
! Output: None.
...
! ******************************************************************************
  ...
  SUBROUTINE ComputePresGasMix(r,e,Y,p,T,a,eEx,eAir)
...
! ******************************************************************************
  ...
  DOUBLE PRECISION, INTENT(INOUT) :: r, e, Y
  DOUBLE PRECISION, INTENT(OUT):: p, T, a, eEx, eAir

! ******************************************************************************
! Locals
! ******************************************************************************
  INTEGER :: iter, iterMax, i, j
  DOUBLE PRECISION :: tol,tolok,smallscale, smallval,vbp,rbp,ebp,pbp,Tbp,f3p,f4p
  DOUBLE PRECISION :: Ja(4,4), InvJa(4,4), dJa(4,4), s(4), f(4), va, vb, ea, eb
  DOUBLE PRECISION :: sol_old(4), f_old(4), sol(4), Jadx(4), df(4)
  DOUBLE PRECISION :: v, Fva, dFdva, Gva, dGdva, sNorm, sNormOld,alpha, sNormsq
  DOUBLE PRECISION :: gammaMix, shcpTNT, dedTcp, dvdTcp
  DOUBLE PRECISION :: ra,rb, pa,pb, Ta,Tb, aa,ab
  DOUBLE PRECISION :: dummyReal
...
!    gammaMix = (shcpLeft*Y + shcpRight*(1.0D0-Y)) &
!              /(shcvLeft*Y + shcvRight*(1.0D0-Y))
!    T = e/(shcvLeft*Y + shcvRight*(1.0D0-Y))
!    p = r*e*(gammaMix-1.0D0)
!    a = SQRT(gammaMix*p/r)
!	eEx =  shcvLeft *T
!	eAir = shcvRight*T
!	RETURN
...
! ******************************************************************************
! Explosive gas (JWL EOS) 
! ******************************************************************************
  IF ( ABS(1.0D0 - Y) <= 1.0D-8 ) THEN 
    CALL JWL1(e,r,p,a,T,3)
    eEx  = e
    eAir = 0.0D0
    RETURN 
  END IF !Y
...
! ******************************************************************************
! Air (Tannehill's equilibrium air curfit)
! ******************************************************************************
  IF ( ABS(Y) <= 1.0D-8 ) THEN   
    CALL tgas1(e,r,p,a,T,3)
    eEx  = 0.0D0
    eAir = e
    RETURN
  END IF !Y
...
! ******************************************************************************
! Explosive gas and air mixture (Broyden's method) 
! ******************************************************************************
! ==============================================================================
! Initial Guess
! ==============================================================================
 
  sol(1) = v !6.213065525944548D0 !v*8.0D0
  sol(2) = v !0.857860966971079D0 !v
  sol(3) = e !9.323363026977179D+05 !e*4.0D0
  sol(4) = e !2.145518769638580D+05 !e
  sol_old(:) = sol(:)

!  Ja(:,:) = 0.0D0	! identity matrix
!  DO i = 1,4
!    DO j = 1,4
!      IF ( i==j ) Ja(i,j) = 1.0D2
!    END DO !j
...
! ==============================================================================
  ...
  DO iter = 1,iterMax
    ...
    ra = 1.0D0/va
    rb = 1.0D0/vb
    CALL JWL1 (ea,ra,pa,dummyReal,Ta,2)   !JWL EOS for product species
    CALL tgas1(eb,rb,pb,dummyReal,Tb,3)   !Curve fit (or could use ideal) EOS for air
    f(3) = pa - pb
    f(4) = Ta - Tb
...
!   Calculate Jacobian matrix approximately
    ...
    dFdva = ATNT*(wTNT/va-R1TNT*rhoTNT+wTNT/R1TNT/rhoTNT/va**2.0D0) &
           *exp(-R1TNT*rhoTNT*va)                                   &
          + BTNT*(wTNT/va-R2TNT*rhoTNT+wTNT/R2TNT/rhoTNT/va**2.0D0) &
           *exp(-R2TNT*rhoTNT*va)
    Ja(3,1)   = -wTNT             *ea/va**2.0D0 + dFdva
    Ja(3,3)   =  wTNT             /va
    ...
    smallscale = smallval*vb
    vbp = vb + smallscale
    rbp = 1.0D0/vbp
    CALL tgas1(eb,rbp,pbp,dummyReal,Tbp,3)
    f3p = pa - pbp
    f4p = Ta - Tbp
    Ja(3,2) = (f3p - f(3))/smallscale
    Ja(4,2) = (f4p - f(4))/smallscale

    smallscale = smallval*eb
    ebp = eb + smallscale
    CALL tgas1(ebp,rb,pbp,dummyReal,Tbp,3)
    f3p = pa - pbp
    f4p = Ta - Tbp
    Ja(3,4) = (f3p - f(3))/smallscale
    Ja(4,4) = (f4p - f(4))/smallscale

    CALL FindInverseMatrix(Ja,InvJa,4,4)
    s(:) = 0.0D0
    DO i = 1,4
      DO j = 1,4
        s(i) = s(i) - InvJa(i,j)*f(j)
      END DO ! j 
    END DO ! i
...
!  IF ( sNorm > tolok ) THEN 
!    WRITE(*,*) 'Broyden Method Iteration not converged!', timeCurrent, sNorm, Y
!    STOP
!  END IF ! iter
! ==============================================================================
! Calculate dependent variables
! ==============================================================================
  ra = 1.0D0/va
  rb = 1.0D0/vb
  CALL JWL1 (ea,ra,p ,aa,T ,3)
  CALL tgas1(eb,rb,pb,ab,Tb,3)
  IF ( ABS(p-pb) > tolok*p .OR. ABS(T-Tb) > tolok*T ) THEN 
    WRITE(*,*) 'Broyden Method Iteration may not be converged!'
...
! ******************************************************************************
  ...
  END SUBROUTINE ComputePresGasMix

! ******************************************************************************
!  
! $Id: RFLU_ModStanley.F90,v 1.1.1.1 2015/01/23 22:57:50 tbanerjee Exp $
!
! Purpose: Thermal and mechanical JWL eos 
!
! Description: JWL1: calculate p, a, T by r and e.
!              mflag=1: return p only; 
!              mflag=2: return p and T;
!              mflag=3: return p, a, and T.
!
!              JWL2: calculate e, a, T by r and p.
!              mflag=1: return e only; 
!              mflag=2: return e and T;
!              mflag=3: return e, a, and T.
!
! Input: None.
!
! Output: None.
...
! ******************************************************************************
  ...
  SUBROUTINE JWL1(e,r,p,a,T,mflag)
...
! ******************************************************************************
  v  = 1.0D0/r
  p = ATNT*(1.0D0-wTNT/R1TNT/rhoTNT/v)*exp(-R1TNT*rhoTNT*v) &
    + BTNT*(1.0D0-wTNT/R2TNT/rhoTNT/v)*exp(-R2TNT*rhoTNT*v) + wTNT*e/v
  IF ( mflag == 1 ) RETURN 
  ...
  a    =  SQRT( rhoTNT*v**2.0D0*( ATNT*R1TNT*exp(-R1TNT*rhoTNT*v)  &
                                + BTNT*R2TNT*exp(-R2TNT*rhoTNT*v)  &
                                + CTNT*(1.0D0+wTNT)/(rhoTNT*v)**(2.0D0+wTNT) ))
  IF ( mflag == 3 ) RETURN 
...
! ******************************************************************************
  ...
  SUBROUTINE JWL2(p,r,e,a,T,mflag)
...
! ******************************************************************************
  v  = 1.0D0/r
  e = p*v/wTNT - ATNT*(v/wTNT - 1.0D0/rhoTNT/R1TNT)*exp(-R1TNT*rhoTNT*v) &
               - BTNT*(v/wTNT - 1.0D0/rhoTNT/R2TNT)*exp(-R2TNT*rhoTNT*v)       !Eq.(26)
  IF ( mflag == 1 ) RETURN
  CTNT = (rhoTNT*e - ATNT/R1TNT*exp(-R1TNT*rhoTNT*v) &
                   - BTNT/R2TNT*exp(-R2TNT*rhoTNT*v))*wTNT*(rhoTNT*v)**wTNT
  T = (p - ATNT*exp(-R1TNT*rhoTNT*v)  &
         - BTNT*exp(-R2TNT*rhoTNT*v))*v/wTNT/shcvTNT                           !Eq.(24)
  IF ( mflag == 2 ) RETURN
  a    =  SQRT( rhoTNT*v**2.0D0*( ATNT*R1TNT*exp(-R1TNT*rhoTNT*v)  &
                                + BTNT*R2TNT*exp(-R2TNT*rhoTNT*v)  &
                                + CTNT*(1.0D0+wTNT)/(rhoTNT*v)**(2.0D0+wTNT) ))
...
SUBROUTINE FindInverseMatrix(matrix,inverse,n,nmax)
implicit none
